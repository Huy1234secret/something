...
function drawNebula(ctx, w, h) {
  const blobs = [
    { x: w * 0.15, y: h * 0.2, r: 280, c1: 'rgba(46,199,255,0.08)' },
    { x: w * 0.85, y: h * 0.25, r: 240, c1: 'rgba(0,255,200,0.07)' },
    { x: w * 0.45, y: h * 0.75, r: 360, c1: 'rgba(64,150,255,0.06)' },
  ];
 ...
 * @property {string} [accent] // e.g. "#24faff"
 */
...
async function generateLevelCard(d) {
  ...
  const FRAME = { m: 10, r: 28, lw: 2 };
  const fx = FRAME.m + 0.5;
  const fy = FRAME.m + 0.5;
  const fw = W - FRAME.m*2 - 1;
  const fh = H - FRAME.m*2 - 1;

  const CONTENT_PAD = 16;
  const cx = fx + CONTENT_PAD;
  const cy = fy + CONTENT_PAD;
  const cw = fw - CONTENT_PAD*2;
  const ch = fh - CONTENT_PAD*2;

  ctx.save();
  roundedRect(ctx, fx, fy, fw, fh, FRAME.r);
  ctx.clip();

  let g = ctx.createLinearGradient(fx, fy, fx + fw, fy + fh);
  ...
  drawNebula(ctx, fw, fh);
  drawStars(ctx, fw, fh, 180);
  scanlines(ctx, fw, fh);
  vignette(ctx, fw, fh);

  const glare = ctx.createLinearGradient(fx, fy, fx + fw, fy + fh);
  ...
  drawCornerBrackets(ctx, fx + 8, fy + 8, fw - 16, fh - 16, 22, 2, 'rgba(36,250,255,0.55)');

  const AV_SIZE = 200;
  const RING_PAD = 36;
  const AV_SAFE_R = AV_SIZE/2 + RING_PAD;

  const avX = cx + AV_SAFE_R;
  const avY = cy + ch / 2;
  ...
  for (let i = 0; i < 100; i += 10) {
    ...
    ctx.strokeStyle = i/100 <= pct ? ACCENT : 'rgba(255,255,255,0.22)';
    ctx.lineWidth = 2;
    ctx.stroke();
  }
  ctx.restore();
  ...
  ctx.fillStyle = '#00aaff'; ctx.fillText(d.username, nameX+1.5, nameY+1.5);
  ctx.fillStyle = '#ff00ff'; ctx.fillText(d.username, nameX-1.2, nameY-1.2);
  ctx.fillStyle = nameGrad;  ctx.fillText(d.username, nameX, nameY);

  const uW = Math.min(cw - (nameX - cx), Math.max(220, ctx.measureText(d.username).width + 36));
  ctx.beginPath();
  ctx.moveTo(nameX, nameY + 12);
  ctx.lineTo(nameX + uW, nameY + 12);
  ctx.lineWidth = 3.5;
  ctx.lineCap   = 'round';
  ctx.shadowColor = ACCENT;
  ctx.shadowBlur  = 12;
  ...
  ctx.fillText(`LEVEL ${d.level}`, nameX, nameY + 48);
  ...
  let chipX = nameX, chipY = nameY + 74;
  const chipH = 36, padH = 14;
  ...
  const bar = { x: nameX, y: cy + ch - 120, w: cx + cw - nameX, h: 40, r: 20 };
  ...
  ctx.save();
  roundedRect(ctx, fx, fy, fw, fh, FRAME.r);
  ctx.lineWidth   = FRAME.lw;
  ctx.strokeStyle = ACCENT;
  ctx.shadowColor = 'transparent';
  ctx.stroke();

  roundedRect(ctx, fx + 1, fy + 1, fw - 2, fh - 2, FRAME.r - 1);
