...
const VOICE_ACTIVITY_INTERVAL_MS = gameConfig.globalSettings?.VOICE_ACTIVITY_INTERVAL_MS || 60000;
...
const DEFAULT_COIN_EMOJI = gameConfig.items.coins?.emoji || '<:JAGcoin:1397581543354142881>';
...
class SystemsManager {
    ...
    initializeSchema() {
        ...
        if (!guildSettingsInfo.some(col => col.name === 'weekendStatusMessageId')) {
            this.db.exec('ALTER TABLE guildSettings ADD COLUMN weekendStatusMessageId TEXT;');
        }
...
`).run(userId, guildId, now);
    ...
    updateUser(userId, guildId, data) {
     ...
     getGuildSettings(guildId) {
        ...
        if (!settings) {
            this.db.prepare(
                `INSERT INTO guildSettings (guildId, coinEmoji, gemEmoji, robuxEmoji, weekendBoostActive, shopRestockDmEnabled, lastWeekendBoostStartAnnounceTimestamp, lastWeekendBoostEndAnnounceTimestamp, weekendAlertMessageId, weekendStatusMessageId)
        ...
        settings.weekendStatusMessageId = settings.weekendStatusMessageId || null;
    ...
    _ensureShopAlertTable() {
        if (this._shopAlertTableEnsured) return;
        this.db.exec(`CREATE TABLE IF NOT EXISTS userShopAlertSettings (
            userId TEXT NOT NULL,
            guildId TEXT NOT NULL,
            itemId TEXT NOT NULL,
            enableAlert INTEGER DEFAULT 1,
            PRIMARY KEY (userId, guildId, itemId)
        );`);
        this._shopAlertTableEnsured = true;
    }
